...
struct HDPositionAlert: View {
    @Binding var dismis: Bool
    @State private var title: String = "Position "
    @State var confirm: (HockeyPlayerTypeModel) -> Void
    var body: some View {
        ZStack {
            if dismis {
                GeometryReader(content: { geometry in
                    VStack {
                        ...
                        ForEach(Array(HockeyPlayerTypeModel.allCases.enumerated()), id: \.element.id) {index, type in
                            if index != 0{
                                Button(action: {
                                    confirm(type)
                                    dismis.toggle()
...
#Preview {
    HDPositionAlert(dismis: .constant(true), confirm: {_ in})
}
